 ...
 *
 * Orchestrates the entire 13-step travel planning process.
 ...
 * 5. Search flights
 * 6. Confirm dates
 * 7. Decide daily concepts
 * 8. Search accommodation
 * 9. Search attractions (for each day)
 * 10. Search restaurants (for each day)
 * 11. Optimize routes (for each day)
 * 12. Create detailed timeline (for each day)
 * 13. Validate and offer adjustments
 */
...
3. **並列実行**: 可能な場合は複数のサブエージェントを並列で実行してください
4. **現実的な計画**: 移動時間、営業時間、疲労度を考慮した実現可能なプランを作成してください

# 利用可能な機能

あなたは以下の機能を使用できます（実際に呼び出す必要はなく、これらの機能があることを前提に会話してください）：

- **フライト検索**: 出発地と目的地間のフライトオプションを検索
- **ホテル検索**: エリアと日程に基づいてホテルを検索
- **観光地検索**: エリアとテーマに合った観光スポットを検索（Web検索機能あり）
- **レストラン検索**: エリアと料理の好みに基づいてレストランを検索（Web検索機能あり）
- **ルート最適化**: 訪問順序を最適化し、詳細なタイムラインを作成
- **距離・時間推定**: 地点間の移動時間と距離を推定
...
export class MainAgent {
  ...
  async chat(userMessage: string): Promise<{
    ...
    // Generate response with Google Search enabled
   ...
   */
  private buildConversationHistory(): Content[] {
